...
function writeStyle(bMiniBar)
{
	...
	*/
	...
	sStyle+=gsBtnStyle;
	sStyle+=".clsToolbarBackground, .minibarcontentsbg{\n";
	sStyle+="margin:2px; \n background-image:url('../Images/contentbutton.png'); background-repeat:no-repeat; }\n";
	sStyle+=".clsToolbarBackground td, .minibarcontentsbg td, .minibarindexbg td, .minibarsearchbg td{\n";
	sStyle+=" }\n";
	sStyle+=".minibarindexbg{\n";
	sStyle+="margin:2px; \n background-image:url('../Images/indexbutton.png'); background-repeat:no-repeat; }\n";
	sStyle+=".minibarsearchbg{\n";
	sStyle+="margin:2px; \n background-image:url('../Images/searchbutton.png'); background-repeat:no-repeat; }\n";
	sStyle+="\n";
	sStyle+=" body {\n";
	if (gsBgImage)
...
function showHelpSystem(id)
{
       ...
       if(strURL != "")
       {
              if(top.frames.length > 0 && top.frames[0].name == "FMXTcontentFrame")
...
function addButton(sType,nStyle,sTitle,sHref,sOnClick,sOnMouseOver,sOnLoad,nWidth,nHeight,sI1,sI2,sI3,sI4,sI5,sI6)
{
	...
	else if(sType.indexOf("custom")==0)
	{
		...
		{
			sButton="<a title=\""+svTitle+"\" id=\"btn"+sType+"\" class=\"btn"+sType+"\" target=\"FMXTcontentFrame\" href=\""+sHref+"\" onclick=\"cusOnClick("+nCusBtnIdx+");";
			bHref=true;
		}
		else
	...
	if(sButton.length!=0)
	{
		...
		{
			if(bHref)
				gaButtons[nBtn]="<td width=\"33%\" NOWRAP valign=\"middle\" align=\"center\" class="+btnClass+" onclick=\"onBtnClick(event); highlightButton(this);\" onmousedown=\"onBtnMouseDown(event, "+nBtn+");\" onmouseup=\"onBtnMouseUp(event, "+nBtn+");\" onmouseover=\"onBtnMouseOver(event, "+nBtn+");\" onmouseout=\"onBtnMouseOut(event, "+nBtn+");\">"+sButton+"</td>";
			else
				gaButtons[nBtn]="<td width=\"33%\" NOWRAP valign=\"middle\" align=\"center\" class="+btnClass+" onclick=\"onBtnClick(event); highlightButton(this);return false;\" onmousedown=\"onBtnMouseDown(event, "+nBtn+");\" onmouseup=\"onBtnMouseUp(event, "+nBtn+");\" onmouseover=\"onBtnMouseOver(event, "+nBtn+");\" onmouseout=\"onBtnMouseOut(event, "+nBtn+");\">"+sButton+"</td>";
		}
		else
			gaButtons[nBtn]="<td width=\"33%\" NOWRAP valign=\"middle\" align=\"center\" class="+btnClass+" state=\"up\" onclick=\"onBtnClick(event); highlightButton(this);return false;\" onmousedown=\"onBtnMouseDown(event, "+nBtn+");\" onmouseup=\"onBtnMouseUp(event, "+nBtn+");\" onmouseover=\"onBtnMouseOver(event, "+nBtn+");\" onmouseout=\"onBtnMouseOut(event, "+nBtn+");\">"+sButton+"</td>";
		gaTypes[nBtn]=sType;
	}
...
Added on 21 June 2011
*/
function highlightButton(elm)
{
	var title = elm.children[0].title;
	
	var parent=elm.parentNode;
	while(parent.tagName.toLowerCase() != "table" || !parent)
	{
		parent = parent.parentNode;
	}
	if(parent.tagName.toLowerCase() == "table")
	{
		parent.className = "minibar"+title.toLowerCase()+"bg";
	}
...
function writeToolBar()
{
	...
	if(gaButtons.length>0)
	{
		...
		if (strHeight<25) strHeight = 25;

		
		if(gbIE)
			sHTML="<form name=\"searchForm\" method=\"POST\" action=\"javascript:searchB()\"><table id=\"whtbar_mbars\" class=\"clsToolbarBackground\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" height=\""+strHeight+"\" width=\"230\">";
		else
			sHTML="<table id=\"whtbar_mbars\" class=\"clsToolbarBackground\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" height=\""+strHeight+"\" width=\"230\">";
...
function showToc()
{
	...
	SendMessage(oMsg);
	if(customHighlightMbar) customHighlightMbar(1);
}
...
function showIndex()
{
	...
	SendMessage(oMsg);
	if(customHighlightMbar) customHighlightMbar(2);
}
...
function showFts()
{
	...
	SendMessage(oMsg);
	if(customHighlightMbar) customHighlightMbar(3);
}
...
function window_OnLoad()
{
	...
	{
		setTimeout( function() {
			chosen_mbar = oMsg.oParam;
			flipPaneButton(chosen_mbar);
		},1);
	...
	if(customTrackHash) customTrackHash();
				...
				if(sState=="up")
				{
					...
					if(sPF=="clsBtn")
					{
						...
						if(oaA.length>0)
						{
							...
							if(strClassName.search("btnsel")<0)
...
if(window.gbWhUtil&&window.gbWhMsg&&window.gbWhVer&&window.gbWhProxy)
{
	...
	goTextFont=new whFont("Verdana","8pt","#919191","normal","normal","none");
