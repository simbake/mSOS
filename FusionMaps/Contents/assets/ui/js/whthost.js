...
var gsABgColor="#E9E9E9";

var giBaseIcon ="../../../Images/base.gif";
var giBookClose="";
var giBookOpen="";
var giBookItem="";
var giURLItem="";
...
/* [start] added by sudipto 14 June */
var listOfDeadEndBooks ={};
var expandedBook ={};
/* [end]   added by sudipto 14 June */
...
function setActiveBgColor(sBgColor){gsABgColor="#DFDFDF";}
...
function insertBookItems(nIdx,num)
{
	...
	if(eChildDiv){
		if((eChildDiv.childNodes&&eChildDiv.childNodes.length==0)||
			...
			var parents = getLineage(nIdx)
			addIndentedLines(sChildBookId,parents);
...
function addIndentedLines(elm, parents)
{
	
	var childNodes = document.getElementById(elm).children;
	
	for(var i=0;i<childNodes.length;i++)
	{
		var grandChild = null;
		var child = childNodes[i].children && childNodes[i].children[0] ? childNodes[i].children[0] : null;
		if(child){
			if(child.tagName.toLowerCase() == "nobr")
				grandChild = child;
			else
				grandChild = child.children && child.children[0] ? child.children[0] : null;
		}
		if(grandChild)
		{
			var linesHTML = "";
			for(var l=parents.length-1;l>0;--l){
				
				if(listOfDeadEndBooks[parents[l]])
				{	//"Contents/Images/"+
					linesHTML +="<img border=0 src='../../../Images/empty.gif' width='18' height='18' align='absmiddle' />";
				}
				else
				{
					linesHTML +="<img border=0 src='../../../Images/line.gif' width='18' height='18' align='absmiddle' />";
				}
				
			}
			grandChild.innerHTML  = linesHTML + grandChild.innerHTML;	
...
function writeBook(nIdx, isLast)
{
	...
	if(bLocal)
	{
		if (!isBookEmpty(nIdx))
		{
			...
			var iconImage = isLast?"../../../Images/plusbottom.gif":"../../../Images/plus.gif";
			...
			sHTML+="); expandCollapse('"+nIdx+"');return false;\" title=\""+sName+"\">";
			
			
			if(nIdx>0)
			{
				sHTML+="<img border=0 id=\"expand"+nIdx+"\" src='"+iconImage+"' width='18' height='18' alt='+' align='absmiddle' />"; // plus-minus
			}
			sHTML+="<img alt=\"Book\" name=\""+getBId(nIdx)+"\" src=\"../../../Images/folder.gif\" border=0 align=\"absmiddle\">"; // book
...
{
	var elm = document.getElementById("expand"+id);
	if(!elm) return;
	
	var tmpStore = elm.src.split(/[\/\\]/);
	var lastImage= tmpStore.pop()
	tmpStore = tmpStore.join("/")+"/";
	
	if(expandedBook[id]!=null)
	{
		
		elm.src = tmpStore + lastImage.replace(/minus/, "plus");
		expandedBook[id] = null;
	}else
	{
		elm.src = tmpStore + lastImage.replace(/plus/i, "minus");
		expandedBook[id] = id;
	}
}
...
function getLevel(nIdx)
{

	var level=0;
	var obj = getElement(getItemId(nIdx));
	if(!obj) obj = getElement(getBookId(nIdx));
	var parentRootReached = false;
	var pObj = obj;
	
	while(!parentRootReached){
		var parentItemFound=false;
		while(!parentItemFound) {
			pObj=getParentNode(pObj);
			
			if( !pObj  ) { 
				parentRootReached=true; 
				break; 
			}
			//isPBId//isCBId
			if( pObj.id && (isCBId(pObj.id)||isPBId(pObj.id))) { 
				parentItemFound=true; 
				level++; 
				
				if(getIdByPBId(pObj.id)==0)
				{
					parentRootReached= true;
				}
...
function getLineage(nIdx)
{
	...
	while(!parentRootReached){
		...
		while(!parentItemFound) {
			...
			if( pObj.id && (isCBId(pObj.id)||isPBId(pObj.id))) { 
				parentItemFound=true; 
				level.push(getIdByPBId(pObj.id)); 
	...
	if(num>0){
		...
		do{
			...
			if(nItemType==1||nItemType==4||nItemType==8){
				...
				// last book
				if(num-nCIdx==0){
					// added the id of the book to the array containing the list of deadend books
					listOfDeadEndBooks[i+""]=i;
				...
				if(num-nCIdx==0)// last page
					sHTML+=writeAnItem(i, true);
				else
					sHTML+=writeAnItem(i);// non-last page
			}
...
function writeAnItem(nIdx, isLast)
{
	...
	var sHTML="<p><nobr><img border=0 src='"+(isLast?"../../../Images/joinbottom.gif": "../../../Images/join.gif")+"' width='18' height='18' alt='-' align='absmiddle' /><a id=\""+getItemId(nIdx)+"\" onfocus =\"markItem("+nIdx+");\" onclick=\"markItem("+nIdx+");\""
	var sAltString="";
	if(nItemType&2)
		sAltString="Page";
	else
	...
	sHTML+="href=\""+_textToHtml_nonbsp(getItemURL(nIdx))+"\" title=\""+sName+"\"><img alt=\""+sAltString+"\" src=\"../../../Images/page.gif\" border=0 align=\"absmiddle\">&nbsp;"+sName+"</a></nobr></p>";
...
{
	return"<div id=\""+gsLoadingDivID+"\" class=parent onclick=\"removeLoadingDiv();\" style=\"padding:4px; margin:5px;background-color:#E9E9E9;border:1px solid #DFDFDF;width:220px;;font-family:Verdana;	\">"+gsLoadingMsg+"</div>";
}
...
function TocWriteClassStyle()
{
	...
	if(gsBgImage)
		...
		sStyle+="body {border-top:none; padding:2px; margin:2px; margin-left:10px;}\n";
	...
	sStyle+="A { padding:2px; padding-left:0; outline: none;}\n";
	sStyle+="A:link {"+getFontStyle(goFont)+" white-space: nowrap;}\n";
...
{
	if (nIdx==0) return _getFullPath(gaProj[0].sPPath,giBaseIcon);
