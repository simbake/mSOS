...
function FtsWriteClassStyle()
{
	...
	if (gsBgImage)
		sStyle+="body {border-top:"+gsBgColor+" 0px solid;}\n";
	else if(gbAIR)
		sStyle+="body {border-top:white 0px solid;}\n";
	else 
	    sStyle+="body {border-top:black 0px solid;}\n";
	sStyle+="P.msg {"+getFontStyle(goErrFont)+"margin-left:"+gsIndent+";margin-top:"+gsMargin+"; margin:10px; margin-top:0; text-align:center;}\n";
	...
	sStyle+="A.fr_tc:active {"+getFontStyle(goHoverFont)+"font-weight:bold;}\n";
	sStyle+="A.fr_tc:focus {"+getFontStyle(goHoverFont)+"font-weight:bold;}\n";
	
	sStyle+="A.fr_act {}\n";
	sStyle+="A.fr_act:link {"+getFontStyle(goFont)+"font-weight:bold; color:#F89527;}\n";
	sStyle+="A.fr_act:visited {"+getFontStyle(goFont)+"font-weight:bold;color:#F89527;}\n";
	sStyle+="A.fr_act:hover {"+getFontStyle(goFont)+"font-weight:bold;color:#F89527;}\n";
	...
	sStyle+="P.pb_tt {"+getFontStyle(goErrFont)+"padding:0;margin:5px; }\n";
	sStyle+="P.pb_in {"+getFontStyle(goErrFont)+"position:relative;padding:0;margin:0;left:0%;top:0%;height:15px;border-style:none;background-color:#B9B9B9;}\n";
	sStyle+="DIV.pb_out {width:100%;border-style:solid;border-width:1px;padding:0;margin:5px;background-color:#F3F3F3; border-color:#8A8888;}\n";
	sStyle+="p.pb_cancel { color:#3B3B3B; cursor:pointer;}\n";
	sStyle+="</STYLE>";
...
function writeTableHead()
{
	if(!gsHideRankColumn)
	{
		if(gbAIRSSL)
		{
				return	"<TABLE CELLSPACING=0 CEllPADDING=0 STYLE='table-layout:fixed; width:99%'>" +
						"<THEAD>" + 
...
function displaySearchProgressBar( a_nProgress )
{
	...
	if( !pb || !pt )
	{
		var sHTML = "<div style='padding:10px;'><P ID='SearchProgress' CLASS='pb_tt'>" + gsSearching + " " + a_nProgress + "%</P>\n" +
					"<DIV CLASS='pb_out'>\n" +
					"<P ID='SearchProgressBar' CLASS='pb_in' STYLE='width:" + a_nProgress + "%'></P>\n" +
					"</DIV>\n" +
					"<CENTER><p CLASS='pb_cancel' ONMOUSEDOWN='context.stop()'><u>"+gsCancel+"</u></p></CENTER></div>\n";
		changeResultView( sHTML );
	}
	else
	{
