...
function showMapFromList(mapName, mapIndex, isMenuItemClicked) {
    ...
    loadScriptFile(window.MARKER_DATAPATH+mapDetails["markers"]+".js", function () {
        window.MAP_JSON = markerJSON;
        window.MAP_JSON['map'] = getThemedMapAttributes();
        window.MAP_JSON = JSON.stringify(window.MAP_JSON, undefined, 2);
        window.MAP_XML = FusionCharts.transcodeData(window.MAP_JSON, 'json', 'xml').formatXML() ;

		updateTabs (mapDetails, mapName);
...
function updateTabs (mapDetails, mapName) {
    
    var targetFunctionList = {"xml":showData, "json":showData, "javascript":renderMap, 'flash':renderMap};
    var targetHTMLElm = {"xml":"mapDataContainer", "json":"mapDataContainer", "javascript":"mapContainer", 'flash':"mapContainer"};
    
    var selectedTab = $("#body #middle #hoverDetails .selected").attr ('name').replace(/map|view/ig,"");
    
    if ($("#body #middle #"+targetHTMLElm[selectedTab]).is(":hidden")) {
        $("#body #middle #"+targetHTMLElm[selectedTab]).show();
    }
    
    if (selectedTab == "xml" || selectedTab =="json") {
        $("#body #middle #mapContainer").hide();
    }else {
        $("#body #middle #mapDataContainer").hide();
    }
    
    window.SELECTEDMAPNAME = mapName;
    window.SELECTEDMAPDETAILS = mapDetails;
    
    targetFunctionList[selectedTab] (mapDetails["swf"], mapName, selectedTab);
    
    
}

function showData (t1, mapName, datatype) {
    $('#body #middle #mapDetails #name').text(mapName +" ");    
    var datasource = {"xml": window.MAP_XML, "json": window.MAP_JSON};
    $("#body #middle #mapDataContainer pre.prettyprint").text( datasource[datatype]);
    prettyPrint();
...
function renderMap(mapSWF, mapName, forceRenderer)
{
    
    if(window.MAPEXPLORER_RENDERER) {
        var renderer = window.MAPEXPLORER_RENDERER;
    ...
    if (forceRenderer) {
        FusionCharts.setCurrentRenderer(forceRenderer);
    }
...
function setDefaultRenderer(){
    ...
    window.NO_FLASH  = /MAPEXPLORER_RENDERER=javascript/i.test(document.cookie);

    if (window.querystring['renderer']) {
        window.NO_FLASH = window.querystring['renderer'].toLowerCase() == 'javascript';
        window.querystring['renderer'] = null;
    }
...
function getRootMapTitleControls() {
    var html = '<input type="text" id="searchthismap" value="" /></div>';
    return html;
}
...
function initMapList(){
    ...
    for(var i in mapList){
        ...
        for(var i=0; i<window.mapListSearchKeys.length; i++) {
            ...
            txt +='<div class="maplistitem" title="' + xpath + '" rel="' + duplicateCount + '"><a>' + window.mapListSearchKeys[i] + '</a></div>'    ;
        }
...
function selectMap() {
	// select map
	var selectedMap = "World With Countries";
    ...
    if(window.querystring['map'] && window.mapListSearchIndex[window.querystring['map'].toLowerCase()]) {
        ...
        selectedMap = $("#body #left #content #container #mapList .maplistitem:first").text().trim();
    }

	// select tab
    var selectedTab = "viewxml";

	if(window.querystring['view']) {
		
		if (window.querystring['view'].toLowerCase() == "map") {
			selectedTab = "view" + window.MAPEXPLORER_RENDERER.toLowerCase()+ "map";
			if ($("#body #middle #hoverDetails a[name="+selectedTab+"]").length==0) {
				selectedTab = "viewxml";
			}
		}
    } 
    ...
    if(window.querystring['tab'] && $("#body #middle #hoverDetails a[name="+window.querystring['tab'].toLowerCase()+"]").length==1) {
        selectedTab = window.querystring['tab'].toLowerCase();
    }
    window.MAPVIEW_TAB = selectedTab;
    $("#body #middle #hoverDetails a[name="+window.MAPVIEW_TAB+"]").addClass('selected');
    document.cookie = "MAPVIEW_TAB="+window.MAPVIEW_TAB;

    $(".openinnewwindow").attr ('href', window.location.href);
    showMapFromList(selectedMap, selectedMapIndex);
}


function showCompactView() {
        $("#hoverDetails").css ( {right: "10px"});    
        $("#header, #left, #fullscreenbtn").remove();
        $("#wrapper #body #content").css( {"margin-top": "0"} );
        $("#middle #content #container").css( {"border-radius": "0" , "right": "10px", "left":"10px", "top": "0", "bottom": "-10px"} );
        
        if ($.browser.msie && parseInt($.browser.version, 10) <= 8) {
            $("#middle").css( { "right": "-10px", "left":"-15px", "top": "-5px", "bottom": "-10px"} );
            if (parseInt($.browser.version, 10) < 8) {
                $("#body #middle").find("#content").height( $("body").height() ); 
            }
        }
        
        window.MAPVIEW_INFRAME = true;
...
function initDimensions() {
    ...
    if( ( window.querystring["compact"]) || 
        ( navigator.userAgent.search(/MSIE/)>=0 && top !== self ) || 
        ( navigator.userAgent.search(/MSIE/)<0 && window.parent && window.parent !== window )
    ) {
...
function swapfullscreen(isAnimate) {
    ...
    if (window.MAPVIEW_FULL) {
        if (isAnimate) {
             if ( $.browser.msie && parseInt($.browser.version, 10) < 8 ) {
                $('#body #middle').animate( {"left": "-10px", "top": (-1*$("#header").height()-10) + "px", "bottom": "-10px"} ,'slow');
             ...
             if ( $.browser.msie && parseInt($.browser.version, 10) < 8 ) {
                 $('#body #middle').css({"left": "-10px" , "top": (-1*$("#header").height()-10) + "px", "bottom": "-10px"});     
        ...
        if (isAnimate) {
            $('#body #middle').animate( {"left": $('#body #left').width()+"px", "top": "0", "bottom": "0"} ,'slow');
            $("#body #middle").find("#content").animate( {"height": $("body").height()-$("#header").height()- 10+"px"});
        }else {
            $('#body #middle').css({"left": $('#body #left').width()+"px", "top": "0", "bottom": "0"});
...
function fitMapStage(isAnimation) {
    ...
    if (isAnimation) {
        $('#body #middle').animate( {"left": $('#body #left').width()+"px" } ,'slow');
    }else {
        $('#body #middle').css({"left": $('#body #left').width()+"px" });
    }
...
function initClicks() {
    ...
    $("#body #middle #hoverDetails a").click(function() {
        $("#body #middle #hoverDetails a").removeClass('selected');
        
        $(this).addClass('selected');
        
        var targetHTMLElm = {"xml":"mapDataContainer", "json":"mapDataContainer", "javascript":"mapContainer", 'flash':"mapContainer"};

        var selectedTab = $(this).attr('name').replace(/map|view/ig, "");
        ...
        if (selectedTab == "xml" || selectedTab =="json") {
            $("#body #middle #mapContainer").hide();
            showData(window.SELECTEDMAPDETAILS['swf'], window.SELECTEDMAPNAME, selectedTab);
         }else {
            $("#body #middle #mapDataContainer").hide();
    
           renderMap(window.SELECTEDMAPDETAILS['swf'], window.SELECTEDMAPNAME, selectedTab);     
        }
...
function loadScriptFile (file, callback, scriptText) {
